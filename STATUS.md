...
**Last updated:** 2026-08-26 (third pass — see §11-12) · **Phase:** 15 of 15 — every phase in the specification is
built. The budget impact model, the comparator intelligence layer and the outcomes and
payer-fit layer are all complete. **Phases 1–15 complete** (§10 for the last four) ·
**Deadline:** 2026-09-06
...
├── docs/CONVENTIONS.md          engineering conventions; ten non-negotiables
...
│
├── docs/standards/
│   ├── backend.md               how to write backend code
│   └── frontend.md              how to write frontend code
│
├── data/
...
## 10. Phases 12–15 — the outcomes and payer-fit layer. Done.

Every requirement in the HEOR review's input taxonomy and dashboard is now built; the
coverage map in `ARCHITECTURE.md` §4B is updated and checkable rather than asserted.

**Run it:** `./run.sh` — API on :8077, interface on :5173.

### 10.1 What was added

| | |
|---|---|
| **M16 — clinical outcomes** | `treatment_effects`, `subgroup_event_rates`, `event_costs`, `response_profiles`, seeded from SELECT (NCT03574597), SURMOUNT-1 (NCT04184622) and SCALE (NCT01272219). The pure engine already existed; this is the data and the wiring. |
| **M17 — payer perspective** | `scenarios.perspective` and `covered_population`. PMPM against covered lives, break-even price by bisection on the real forward pass, and the same scenario at 0.5x / 1.0x / 1.75x uptake. |
| **M18 — disease subgroups** | Six obesity segments partitioning the diagnosed population. The engine runs unchanged once per segment; `services/segmentation.py` puts the results back together. |
| **M19 — workbook import** | Template out (pre-filled with the live price grid), filled workbook in, parsed to a reviewable draft. Never saves anything. |
| **Editable prices** | The whole therapy x market grid, every cell editable, derived cells marked as derived. An edit becomes a scenario override, never a write to `drug_prices`. |
| **Interface** | Inputs ordered by the model's own computation sequence with in-place hover help; results as tabs rather than one scroll; with/against-current-care comparison charts. |

### 10.2 The decisions worth not re-deriving

**A baseline event rate belongs to the population, never to the therapy.** This is the
whole reason `subgroup_event_rates` is a separate table from `treatment_effects`. SELECT's
hazard ratio of 0.80 is a property of semaglutide; the 2.4% annual MACE rate it applies to
is a property of secondary prevention, and it is roughly ten times the rate in obesity
without cardiovascular disease. Transporting an *absolute* reduction between populations is
the commonest way a budget impact model overstates what a therapy buys, and the schema
makes it impossible to do here.

**A subgroup is a scenario dimension, not an engine contract.** `EngineInputBuilder`
expresses a segment as two additional criteria — its share of the diagnosed population, and
its eligibility adjustment — plus a multiplier on the uptake curve. `biet_engine` is never
told segments exist. This is the cheap design M3 §12 hoped for and STATUS previously
recorded as too expensive; it cost no engine change at all.

**Funnel aggregation across segments is not a uniform sum, and the failure mode is quiet.**
Segments partition the *diagnosed* population, so every stage from total population down to
treated is identical in each one — summing them across six segments reports six times the
country's population while leaving the bottom of the funnel correct. `segmentation.py`
derives which stages sum from `FunnelStage`'s declared order rather than listing them, so
adding a stage cannot silently leave it behind.

**The single-input analyses run over a combined segment.** A tornado, a Monte Carlo, a price
corridor and a break-even solve are each defined over one `EngineInput`, and six of them is
not an answer. `OutcomesService.combined_segment` builds a share-weighted union so those
analyses describe the same population as the headline. Found by running a two-segment
scenario and seeing a credible interval that did not contain its own point estimate.

**An un-segmented run still needs baseline rates.** Caught the same way. With no segments
selected the outcomes panel reported "no treatment effect is published", which is false —
the effect is published, it was the *population* that had no rate attached. The whole
diagnosed population's rate is the share-weighted average of its segments', and a segment
with no row for an event contributes a **zero rather than an absence**: the
obesity-with-diabetes segment cannot produce new cases of diabetes, so averaging only over
the susceptible and applying that to everybody would overstate avoided cases by the share
never at risk. Verified: MACE resolves to 0.79% a year and incident diabetes to 1.46%,
matching the share-weighted arithmetic exactly, and `BASELINE_RATE_PARTIAL` reports that
the diabetes rate was observed in 81% of the population.

**Break-even is bisection, not the analytic solve.** M8's decomposition rests on budget
impact being linear in price, which it is — but the break-even bracket spans zero, and at a
low enough price both the displacement floor and the PPP floor can bind and break that
linearity. Eighty bisection steps on a sub-millisecond forward pass costs nothing
measurable. Two markets legitimately have *no* break-even price and say which and why.

**One field dictionary, four consumers.** `constants/field_guide.py` is the single
description of every input. The hover text in the interface, the guidance column in the
import template, the Guide sheet in the exported workbook and the message on a rejected
cell are all literally the same string. Four copies of a sentence is four chances to
disagree, and a field described one way on screen and another in the workbook is worse than
one described nowhere.

### 10.3 Engine precision

Exact summation (`math.fsum`) replaces sequential addition wherever error accumulates:
M7's two-world costs (a small number found between two large ones, so rounding in the
addends shows up magnified in the difference), the cross-market and cross-year totals, M8's
affordability ratio, and — most consequentially — M4's share-accounting invariant, which is
asserted against a 1e-9 tolerance and could previously fail for no reason but the order the
therapies happened to be in. `Money.total` carries the currency check, so exactness did not
cost the type's one guarantee. `derive_ppp_price` skips `pow` at the default elasticity of
1.0, where `x ** 1.0` is not guaranteed to return `x` bit-for-bit and every unpriced market
takes that path.

The net effect is that a run reordering its markets returns bit-identical totals, which is
what non-negotiable 9's "a run reproduces exactly" actually requires.

### 10.4 Two interface defects found by driving it rather than reading it

**Successive toggles lost all but the last.** The input panel took a value callback, so
selecting six segments inside one React batch built each update from the same stale draft.
It now takes the dispatcher and every update is functional.

**One wide table scrolled the whole page sideways.** A grid item's default `min-width: auto`
refuses to shrink below its content, so the ten-market price grid pushed its column past the
viewport and took the sticky input panel off screen with it. `minmax(0, 1fr)` plus
`min-width: 0` lets the table scroll inside its own container and the page never does.

Also: the sticky header's height is measured rather than guessed. It wraps from one row to
three between a monitor and a phone and grows again when a result arrives, so no constant is
right at every width — and one that is too small hides content under an opaque bar.
`ResizeObserver` turned out not to fire at all in the headless browser this was verified in,
so the hook measures in a layout effect and on resize instead, which is more portable
anyway.

### 10.5 What the numbers say

Wegovy, five markets, three years, whole diagnosed population, EUR:

```
cumulative incremental impact   EUR 2.42bn      peak year 3
patients treated, Y3            272,404         of 2,116,216 addressable
cost per treated patient        EUR 4,104
events avoided                  5,577           EUR 580.7m of care avoided
break-even price, USA           $49/unit        against $141 today — a 65% cut
break-even price, GBR           +42% headroom   already saving there
PMPM, insurer at 4m lives       EUR 23.29       final year
```

The six segments carry the cost very unevenly against what it buys: obesity with established
cardiovascular disease is 12% of the population and avoids the most events per patient
treated by an order of magnitude, while obesity without cardiometabolic comorbidity is 20%
of it and avoids the fewest. Cost per patient barely moves between them, because it is a
price. That contrast is the case for restricting eligibility, and it is invisible in any
un-segmented result.

### 10.6 Still open

- **Two non-drug comparators** — a lifestyle programme and bariatric surgery, named in
  Phase 12's plan. The comparator basket is drug-only.
- **`scripts/demo_data.py purge`** before any number reaches a slide (§5.5), unchanged.
- **The `data/` lint findings** (16, pre-existing) and the IBAB password rotation (§6),
  both unchanged by this work.

504 tests, mypy --strict clean across 82 source files, ruff clean, tsc clean, production
build clean.

---

## 11. WHO data, epidemiology terminology, and the interface reorder

Driven by two WHO-derived CSVs supplied for this round and a written
specification for how epidemiology should read in a budget impact dashboard.

### 11.1 The finding that changed the design

**The WHO-derived obesity subgroups overlap.** Across the ten source countries
the four shares — diabesity, established cardiovascular disease, hypertension
or sleep apnoea, and severe obesity — sum to **1.33x to 1.61x** the obese
population, because a patient with both diabetes and hypertension is counted in
two of them. They are *alternative eligibility definitions to compare*, not
components of a total to add.

That is the opposite of the six hierarchical segments this system carried
before, which partitioned the population and aggregated correctly. So:

- `disease_subgroups.is_overlapping` records which kind a set is, and the seed
  publisher applies the sum-to-1 check **only** to partitions. Rejecting the
  WHO file for summing to 1.5 would be rejecting it for being correct.
- Selecting more than one overlapping subgroup is **refused** by the API, not
  summed. Summing them would overstate the treated population by half with
  nothing erroring. The interface offers a single choice rather than a
  multi-select it would then have to refuse.
- `obesity_all` is a real row — the whole prevalent population, flagged
  `is_default_population`. It is the denominator the others are shares of and
  the population a run models when nothing is selected, so it carries the
  population-level event rates and is excluded from the selectable list.

**Subgroup shares are per market.** WHO puts hypertension prevalence at 34.0%
in Brazil and 26.2% in France, and the hypertension subgroup's share of obese
adults tracks it — 56.1% against 43.2%. `subgroup_country_rates` carries the
market-specific figure; a market with no row falls back to the cross-market
mean at a weaker tier, and says so.

### 11.2 Prevalence and incidence are now separate quantities everywhere

They were conflated, and they differ by more than an order of magnitude:
US adult obesity is **42.9% prevalent** against roughly **1,716 new cases per
100,000 a year**. Reading one as the other misstates the addressable population
by a factor of twenty-five.

- `country_health_indicators.indicator_kind` records whether a figure is a
  prevalence, an incidence, a coverage or a categorical policy status. Nothing
  downstream infers it from the name.
- WHO publishes prevalence, not incidence. Incidence is **derived** as
  `prevalence / mean duration` under the steady-state relation `I = P / D`, and
  every row says so in its own source string at tier C. It is an input to argue
  with, not an observation.
- Incidence applies to the population **at risk** — adults who do not already
  have the condition. Applying it to all adults would count new cases among
  people who already have it.
- The interface carries them as two separate cards, in two separate units, with
  the arithmetic under each.

### 11.3 The interface

**Tab order is now Population funnel, Affordability, Market access,** then
budget impact and everything behind it — the order a formulary conversation
actually runs in, rather than the order the modules were built in.

**The funnel shows its arithmetic.** Every step reports the multiplier it
applied and the multiplication written out — `271,650,604 × 41.0% =
111,265,371` — with an `ⓘ Source` control giving the confidence tier, vintage,
resolution level and citation. A funnel that shows only its outputs cannot
answer the question it exists to answer, which is *where did the patient number
come from*.

**Every result states its sign in words.** `−£28,757,333` is read as a large
cost by anyone skimming, and the difference between a cost and a saving is the
difference between two opposite recommendations. The interpretation panel says
which, notes when markets disagree and why that is normal, names the peak year,
and — where a saving rests on a mixed price basis or an excluded comparator —
says the saving may be an artefact of the comparison rather than a finding.

**Charts and tables say "Without intervention" and "With intervention"**, and a
current-scenario-against-new-scenario table sets the two side by side with the
incremental figure as the difference between two columns.

**Typography tightened throughout.** The previous scale was set like a magazine
spread — 14.5px serif body at 1.65 line-height in a mostly empty column. Padding,
gaps and type sizes are down roughly 25-30% across cards, panels and the input
studio, and the build screen now carries WHO's burden table for the selected
markets rather than white space.

### 11.4 Comparator placeholders

The two non-drug comparators the HEOR review named — a structured lifestyle
programme and bariatric surgery — are seeded as **deliberately unpriced
placeholders**, tier D, with the placeholder status written into the source
string.

An unpriced comparator used to raise `UnpricedReferenceError` deep in price
derivation and break every calculation. It is now excluded from the
world-without **by name**, with `COMPARATOR_UNPRICED` stating that budget impact
is overstated by exactly what that therapy costs today. A placeholder that
breaks every calculation is not a placeholder; a silently dropped comparator is
the failure M12's guard exists to prevent. Naming it is neither.

### 11.5 A stale-data defect this surfaced

`upsert` only ever adds and updates, which is right for a catalogue that grows.
But the six old subgroups stayed in the database after being removed from the
seed file, and stayed **selectable** — a scenario could have been built on a
definition no longer in any source. `publish_disease_subgroups` now prunes rows
absent from the file. Worth checking for any other table whose CSV is the
complete authority rather than an increment.

### 11.6 Verified

WHO figures reach the screen unchanged: USA obesity prevalence 42.9%, diabetes
11.3%, hypertension 32.5%, derived incidence 1,716 per 100,000 — each matching
`who_gho_multi_indicator_deep_database.csv` exactly. The Excel round trip
carries the new fields (subgroup, prevalence, perspective, covered lives)
cleanly. Nine tabs render with content, no console errors, no horizontal
overflow from phone to 1440px.

473 tests, mypy --strict clean across 83 files, ruff clean, tsc clean, alembic
reports no drift.

### 11.7 Not done

- **The ChatGPT share link could not be read** — it resolves to an unrelated
  page ("Learn scRNA seq basics"). The two CSVs and the pasted specification
  were used instead.
- **The two non-drug comparators carry no price**, deliberately (§11.4). They
  are visible as a named gap rather than costed on an invented figure.
- **Three markets are outside the WHO file.** It covers Canada, Australia and
  Mexico, which this model does not, and omits Italy, Spain and Japan, which it
  does. Those three fall back to the model's own epidemiology table, and the
  build screen names them rather than leaving the row out.

---

## 12. Visual refinement, the brand mark, and dynamic imported inputs

### 12.1 Why the workspace order is Build -> Prices -> Comparators -> Results

Asked directly, so recorded rather than left implicit. The order is a
**dependency chain**, not a menu, and each step is numbered in the header to say
so.

| Step | Why it cannot come earlier |
|---|---|
| **1 · Build** — who and where | Nothing else has a scope until the markets, disease and subgroup are chosen. The price grid does not know which cells to show; the comparator basket does not know which indication to search; the funnel has no population. Every later step is *parameterised by* this one. |
| **2 · Prices** — what it costs | A price is per market, so it needs step 1's market set to exist. The workbook import lives here too, because an import can *change* the market set — and a scenario re-scoped after its comparators were priced would carry prices for markets it no longer covers. |
| **3 · Comparators** — what it displaces | Discovery queries by indication and target, so it needs step 1. Promotion requires a price and a regimen, so it needs step 2 — M12 refuses an unpriced comparator by name. Putting this before Prices would put a step in front of its own precondition. |
| **4 · Results** — what it means | The only step that **reads** rather than writes. Every figure is a function of the three before it, and a result computed before the basket is settled is a result about a different world. |

The deeper reason is the model's own arithmetic: budget impact is *incremental*
— world-with minus world-without — so the world-without has to be complete
before the subtraction means anything. Steps 1-3 build that world; step 4
subtracts.

Within step 4 the tabs run in the order a formulary conversation does:
population funnel, affordability, market access, then the headline and its
working (§11.3).

### 12.2 The brand mark

Two ideas, because the product does two things.

**The funnel** — three bars narrowing downward: a national population narrowing
to the handful of patients a therapy is actually paid for. It is the shape every
user of this tool draws on a whiteboard within two minutes of explaining it.

**The delta** — a wedge notched out of the bottom bar. Budget impact is a
*difference* between two worlds, not a total. The notch is deliberately small
between two large forms, because that is the arithmetic: a small number found
between two large ones. A mark with a big dramatic wedge would misrepresent the
product before anyone had read a figure.

One inline SVG (`shared/BrandMark.tsx`): no request, cannot 404, inherits the
theme's accent, crisp at every size the header wraps to.

### 12.3 Imported columns become editable inputs

The requirement: whatever columns the spreadsheet has should appear in the left
panel and be editable; whatever it does not cover stays on the default.

**The analyst's own file imports unmodified** — no transposing into a template
first. `who_derived_obesity_subgroups_and_eligibility.csv` as supplied reads as
10 rows into **90 per-market values**: prevalence plus all four subgroups' share
and clinical-eligibility fraction, for every market.

Columns are recognised by *pattern*, not a fixed list, because the analyst's
file numbers its subgroups (`subgroup_1_diabesity_cases`) while this model names
them. The regex captures both index and slug, which matters: the cases column
names its subgroup and the eligibility column beside it does not
(`subgroup_1_clinically_eligible`), so the pair is found by index rather than by
deriving one column name from the other. String surgery there produces a column
that does not exist — which it did on the first attempt, and is why the
eligibility values silently failed to import.

Imported values sit in a **"From your file" section** in the input panel, each
labelled in the panel's own vocabulary (`subgroup.htn_osa.share` renders as
"Obesity + hypertension or sleep apnoea — share of prevalent"), tagged with its
market, editable, and droppable individually or all at once. Anything the file
did not cover is not listed and stays on the seeded default.

They reach the engine as ordinary per-market overrides through the closed
vocabulary — bounded and provenanced like any other value, rather than trusted
because they arrived from a spreadsheet. Two paths were added for them:
`subgroup.<code>.share` and `subgroup.<code>.eligible_factor`.

**Verified in the browser**: import the file, edit USA prevalence from 0.429 to
0.50, run — prevalent patients move 111,265,371 -> 135,825,302 and cumulative
impact EUR 544.5m -> EUR 662.7m.

### 12.4 The visual pass

Toward the reference dashboards: KPI cards carrying a sparkline and a signed
delta chip; softer radii (12px) with two-stage shadows that lift on hover;
lighter table rules with more air *inside* rows; pill buttons; numbered
workspace steps. Six KPI cards now fit one laptop row rather than wrapping five
and orphaning the sixth.

The direction throughout is more air inside elements and less between them —
the opposite of the previous pass, which had magazine-spread spacing in a mostly
empty column.

### 12.5 Not done

- **Sparklines appear only where a real series exists** (patients treated,
  impact by year). Cards holding a single point — prevalence, target population
  — have none rather than a decorative flat line.
- **The chart bundle is still 154 kB gzipped**, cached separately from the 35 kB
  app chunk. Splitting recharts further needs per-tab lazy loading, a real
  change to how tabs mount, and was not worth it for an internal tool.
...
12. **M10 — Evidence, Narrative & Export: the pure half is done; the I/O half is Phase 3.**
    ...
    that can live in `biet_engine` (non-negotiable 1), and `biet_api` has no
...
15. ~~**Phase 5 — Evidence, Export and Hardening**~~ — done. **A complete scenario now
    ...
    - **Narrative** — composed deterministically from the engine's own numbers,
      then put through `validate_numbers`. Assembling the prose *out of* the
      result rather than writing it *about* the result is what makes "no figure
      here was invented" a property of the construction rather than a claim
      about it.
