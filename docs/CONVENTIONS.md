...
| Document | Covers |
|---|---|
| [ARCHITECTURE.md](ARCHITECTURE.md) | **The specification.** Scope, modules, formulas, schema, API, delivery plan. |
| [modules/](modules/README.md) | **Per-module specs (M0–M10).** Contracts, logic, edge cases, tests, acceptance criteria. Read your module's spec before building it. |
| [docs/standards/backend.md](standards/backend.md) | How to write backend code |
| [docs/standards/frontend.md](standards/frontend.md) | How to write frontend code |

Working under `backend/` → follow the backend standards. Under `frontend/` → the frontend ones.
Where a standards document and the architecture document disagree, the architecture document wins.
