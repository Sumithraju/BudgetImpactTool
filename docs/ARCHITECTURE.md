...
|---|---|---|
| Population — covered population, age range, annual growth | M17 §5.2, **M2** | **done** — covered lives are a scenario field; growth existing |
...
| Current care — lifestyle programme, existing medicines, bariatric surgery | **M5**, M12 | medicines existing and now editable per market; two non-drug comparators still open |
...
| Treatment behaviour — weight regain | M16 §5.4 | **done** — seeded at zero on the maintenance trials, editable as `outcomes.regain_per_year` |
| Outcomes — weight loss, reduction in diabetes, hypertension, sleep apnoea, CV events | M16 §5.2–5.3 | **done** — `treatment_effects` and `subgroup_event_rates` seeded from SELECT, SURMOUNT-1, SCALE |
| Healthcare costs — drug, consultations, labs, hospitalisations | **M5** | existing |
| Healthcare costs — complication treatment | M16 §5.5 | **done** — `event_costs`, cited for the reference market and PPP-derived elsewhere |
| Time horizon | **M1** | existing |
| Perspective — insurer, employer, government, health system | M17 §5.1 | **done** — a scenario field, with covered lives required for the two subset perspectives |
...
| PMPM impact for the payer | M17 §5.3 | **done** — against covered lives, and flagged on the figure when the denominator was assumed |
| Expected weight-loss responders | M16 §5.2 | **done** — persistence-adjusted, from STEP 1 / SURMOUNT-1 / SCALE |
| Diabetes or cardiovascular events avoided | M16 §5.3 | **done** — per subgroup, since the baseline rate belongs to the population and not the therapy |
| Hospital costs avoided | M16 §5.5 | **done** — enters as M5's offset, never as a separate line |
| Break-even treatment price | M17 §5.4 | **done** — bisection on the real forward pass, per market |
| Budget impact under low, medium, high uptake | M17 §5.5 | **done** — x0.5 / x1.0 / x1.75, stated as a framing device rather than an interval |

All twenty-five are now built. Twelve were already there at the time of the review; the
remaining thirteen are Phases 12–15 below, and the gap they closed was concentrated in one
place — the model priced care and never valued its consequences.
...
- Narrative generation, composed deterministically from the engine's own figures.
