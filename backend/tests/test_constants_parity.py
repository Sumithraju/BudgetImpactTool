...
from biet_api.constants.domain import EventClass as ApiEventClass
...
from biet_api.constants.domain import ResponseThreshold as ApiResponseThreshold
...
from biet_engine.constants import EventClass as EngineEventClass
...
from biet_engine.constants import ResponseThreshold as EngineResponseThreshold
...
def test_event_class_members_match() -> None:
    """M16. `event_costs.event_class` and `subgroup_event_rates.event_class`
    are written on the API side and read as `EventClass` on the engine side.
    A member present in one and not the other is a seeded row the engine
    cannot interpret."""
    assert _members(EngineEventClass) == _members(ApiEventClass)


def test_response_threshold_members_match() -> None:
    assert _members(EngineResponseThreshold) == _members(ApiResponseThreshold)
