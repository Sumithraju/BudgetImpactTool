...
def _year_result(
    *,
    ...
    """
    # `fsum` throughout, not plain `sum`. These are the two quantities the
    # whole model reduces to, they are subtracted from each other, and they are
    # close in magnitude — precisely the arrangement where accumulated rounding
    # in the addends shows up magnified in the difference. Budget impact is a
    # small number found between two large ones; it deserves exact addition.
    cost_without_per_addressable = math.fsum(
        t.m_without * t.persistence * t.annual_cost for t in comparators
    )
    cost_with_per_addressable = math.fsum(
        (uptake * new_persistence * new_annual_cost,
    ...
    net_cost_per_switch = Money(
        amount=math.fsum(
            (new_persistence * new_annual_cost,
             *(-t.sigma * t.persistence * t.annual_cost for t in comparators)),
        ),
        currency=currency,
    )
...
def compute_budget_impact(inputs: EngineInput) -> EngineResult:
    ...
    for index in range(inputs.horizon_years):
        # Summed exactly rather than folded left-to-right: ten markets at
        # very different magnitudes, converted into one currency, is the case
        # where the order countries happen to arrive in would otherwise move
        # the last bits of the total.
        year_total = Money.total(
            (
                convert(
    ...
    cumulative = Money.total(by_year_totals, inputs.reporting_currency)
