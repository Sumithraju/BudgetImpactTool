...
class Money(BaseModel):
    ...
    def total(cls, amounts: Iterable[Money], currency: str) -> Money:
        """Exactly-rounded sum of same-currency amounts.

        `math.fsum` rather than repeated `+`, and the difference is not
        academic here. Budget impact aggregates ten markets across five years
        at magnitudes in the billions while the per-year figures that make it
        up can be six orders of magnitude smaller; sequential addition loses
        low-order bits at every step and the error depends on the *order* the
        markets happen to be in. `fsum` is correctly rounded regardless of
        order, so a run reordering its markets returns bit-identical totals —
        which is what non-negotiable 9's "a run reproduces exactly" actually
        requires.

        The currency check is not relaxed for speed: every amount must already
        be in `currency`, because a silent mixed-currency sum is the one error
        ...
        for money in amounts:
            if money.currency != currency:
                raise CurrencyMismatchError(
                    f"cannot total {money.currency} into {currency}",
                    currency_a=currency, currency_b=money.currency,
                )
            values.append(money.amount)
        return cls(amount=math.fsum(values), currency=currency)
...
class MarketMix(BaseModel):
    ...
    # other channel to surface that (the backend standards, section 8.6).
    warnings: tuple[Warning_, ...] = ()
...
class CountryInput(BaseModel):
    ...
    No optional fields, no defaults (the backend standards, section 3) — with two
...
class EngineInput(BaseModel):
    ...
    # Not in M1's own abbreviated EngineInput snippet. non-negotiable
