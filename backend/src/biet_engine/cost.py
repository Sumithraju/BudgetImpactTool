...
def derive_ppp_price(
    ...
    """
    ratio = gdp_pc_ppp_target / gdp_pc_ppp_reference
    # `pow` is skipped at the default elasticity of 1.0, and this is a
    # precision decision rather than a speed one: `x ** 1.0` is not guaranteed
    # to return `x` bit-for-bit, so the common case would otherwise carry a
    # rounding that the formula does not call for. It is also the case every
    # unpriced market takes, so the exactness is not hypothetical.
    scaled = reference_price * (ratio if elasticity == 1.0 else float(ratio ** elasticity))
