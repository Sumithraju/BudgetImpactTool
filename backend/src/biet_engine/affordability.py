...
def band_for(cumulative_ratio: float) -> AffordabilityBand:
    """Highest threshold the ratio meets or exceeds; LOW below the lowest —
    including every negative ratio (a saving), per section 5.1.

    Public, because a caller that aggregates several results into one ratio
    must reclassify it and must not carry its own copy of the thresholds to do
    so. A boundary that drifted between the code that classifies and the code
    that draws would mislabel a result without either side erroring, which is
    the same reason the API serves these thresholds rather than the interface
    hard-coding them.
    """
...
def compute_affordability(result: EngineResult, inputs: EngineInput) -> tuple[CountryAffordability, ...]:
    ...
    for country_input, country_result in zip(inputs.countries, result.countries, strict=True):
        ...
        ratios: list[float] = []
        bi_usd_by_year: list[float] = []
        budget_usd_by_year: list[float] = []
        for index, year_result in enumerate(country_result.years):
            ...
            bi_usd_by_year.append(bi_usd)
            budget_usd_by_year.append(health_budget_usd)

        # Collected then summed exactly, rather than accumulated in the loop.
        # The ratio of sums is compared against thresholds three orders of
        # magnitude apart from typical values, and a national health budget is
        # a trillion-scale number while a single year's impact can be in the
        # millions — the arrangement where a running total loses the addend.
        bi_usd_sum = math.fsum(bi_usd_by_year)
        budget_usd_sum = math.fsum(budget_usd_by_year)
        ...
        entries.append(
            CountryAffordability(
                ...
                band=band_for(cumulative_ratio),
            )
        )
...
__all__ = ["band_for", "compute_affordability"]
