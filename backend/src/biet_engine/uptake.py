...
def displace(
    ...
    if redistributed:
        ...
        total = math.fsum(headroom.values())
        if total <= 0:
...
def build_market_mix(
    ...
    for index, u in enumerate(uptake):
        ...
        total = math.fsum(m_without.values())
        ...
        # `fsum`, because this line is checked against a 1e-9 tolerance two
        # lines below. With plain summation the assertion measures the order
        # the therapies happen to be in as much as it measures the model, and
        # a ten-therapy mix can fail it for no reason but that.
        accounted = math.fsum((u, *m_with.values()))
        assert abs(accounted - 1.0) <= ACCOUNTING_TOLERANCE, (
            f"share accounting invariant violated: u={u} + sum(m_with)="
            f"{math.fsum(m_with.values())} = {accounted}, expected 1.0"
        )
