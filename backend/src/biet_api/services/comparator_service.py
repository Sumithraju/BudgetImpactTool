    ...
    def discover(
        ...
        *,
        ...
        try:
            ...
            raise UpstreamUnavailableError(
                f"We could not reach the drug database to look up {symbol}. "
                "This is usually temporary — try again in a moment.",
                target=symbol,
            ) from exc
        except Exception as exc:
            # Deliberately broad, and only here.
            #
            # This call parses a live third-party GraphQL response whose schema
            # has already moved once under this code (`knownDrugs` became
            # `drugAndClinicalCandidates`). A field that disappears or changes
            # shape surfaces as a KeyError or TypeError deep in the parse, and
            # an uncaught one becomes a bare 500 — which reaches the user as
            # "Internal Server Error" and tells them nothing they can act on.
            #
            # The real cause is logged at exception level, so nothing is lost
            # for whoever debugs it; what changes is that the reader is told the
            # lookup failed rather than that the application broke.
            log.exception("discovery_parse_failed target=%s", symbol)
            raise UpstreamUnavailableError(
                f"The drug database returned something we could not read for "
                f"{symbol}. This usually means the source changed its format. "
                "Discovery is unavailable until it is updated — everything else "
                "in the tool still works.",
                target=symbol,
            ) from exc
        ...
        if unpriced:
            warnings.append({
                ...
                "message": (
                    f"{unpriced} of these therapies have no price or dosing "
                    "information yet, so they cannot be included in a calculation. "
                    "Searching finds the molecule; it does not find what the "
                    "molecule costs. Add a price and a dose on the Prices step to "
                    "bring them in."
                ),
            })
