...
from .workbook_reader import FindingCollector, column_letter, read_table
...
class ComparatorImportService:
    ...
    def parse(self, data: bytes, filename: str) -> ComparatorImportResult:
        ...
        self._sheet = collector.sheet
        self._findings.extend(collector.findings)
        if table is None:
            ...
            out.append(
                ImportedComparator(
                    ...
                    origin=f"{self._sheet}!"
                           f"{column_letter(index[WorkbookColumn.NAME])}{row_number}",
                )
            )
