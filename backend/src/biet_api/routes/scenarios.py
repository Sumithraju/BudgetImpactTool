...
def _read(scenario: Scenario) -> ScenarioRead:
    return ScenarioRead(
        ...
        covered_population=scenario.covered_population,
        subgroup_codes=list(scenario.subgroup_codes or []),
...
@router.get(
    "/scenarios/{scenario_id}/break-even", response_model=BreakEvenResponse,
)
def break_even(scenario_id: uuid.UUID, session: SessionDep) -> BreakEvenResponse:
    """M17 section 5.4. The price at which incremental impact is zero.

    Separate from `/solve`, which targets an affordability *ratio*. Break-even
    needs no threshold to be agreed first, which is why it is usually the
    number a pricing conversation actually opens with.
    """
    scenario = ScenarioService(session).require(scenario_id)
    return CalculationService(session).break_even(scenario)


@router.get(
    "/scenarios/{scenario_id}/uptake-scenarios", response_model=UptakeScenarioResponse,
)
def uptake_scenarios(
    scenario_id: uuid.UUID, session: SessionDep,
) -> UptakeScenarioResponse:
    """M17 section 5.5. The same scenario at low, medium and high adoption.

    Deliberately not an uncertainty interval — `/psa` is that. These are three
    stated multipliers a reader can argue with, because no source supplies an
    adoption distribution for an unlaunched asset.
    """
    scenario = ScenarioService(session).require(scenario_id)
    return CalculationService(session).uptake_scenarios(scenario)
