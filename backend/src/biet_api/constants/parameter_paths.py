...
PARAMETER_PATHS: Final[tuple[ParameterPathSpec, ...]] = (
    ...
    # M16. Annual proportional loss of the achieved treatment effect. The
    # seeded value rests on trials that maintained weight loss on continued
    # therapy; a payer who does not accept that reading needs to be able to
    # say so and watch every avoided-event count from year 2 fall.
    _literal("outcomes.regain_per_year", PathValueType.FLOAT, **_SHARE),
    # M18. A subgroup's share of the prevalent population and the fraction of
    # it that is clinically eligible, per market. Both arrive from the
    # analyst's own WHO derivation, so both are overridable rather than fixed
    # reference data — the derivation is theirs, not a published figure.
    _templated(
        "subgroup.<subgroup_code>.share", r"subgroup\.[a-z0-9_]+\.share",
        PathValueType.FLOAT, **_RATE,
    ),
    _templated(
        "subgroup.<subgroup_code>.eligible_factor",
        r"subgroup\.[a-z0-9_]+\.eligible_factor",
        PathValueType.FLOAT, **_RATE,
    ),
